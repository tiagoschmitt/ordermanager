...
/**
 * Verify if there are any special sale in the order
 * @param {dw.order.Order} order - The order object to be placed
 */
function checkSpecialSale(order) {
    var orderCampaignId = getSpecialSaleCampaignId(order.priceAdjustments);
    var productCampaignId;

    collections.forEach(order.productLineItems, function (pli) {
        if (pli.priceAdjustments.getLength() > 0) {
            productCampaignId = getSpecialSaleCampaignId(pli.priceAdjustments);

            if (productCampaignId != null) {
                return;
            }
        }
    });

    order.custom.specialSaleCampaign = orderCampaignId || productCampaignId;
}

function getSpecialSaleCampaignId(priceAdjustments) {
    var specialCampaign;

    collections.forEach(priceAdjustments, function (adjusment) {
        if (adjusment.campaign.custom.isSpecialSale) {
            specialCampaign = adjusment.campaign.ID;
            return;
        }
    });
